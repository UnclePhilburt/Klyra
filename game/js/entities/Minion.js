...
class Minion {
    constructor(scene, x, y, ownerId, isPermanent = false, minionId = null) {
        ...
        this.positionUpdateInterval = 150; // PERFORMANCE: Increased from 50ms to 150ms (~7 updates/sec)
        this.lastSentX = null; // For delta compression
        this.lastSentY = null; // For delta compression
    ...
    setupAI() {
        // PERFORMANCE: Only run AI for local player's minions (safety check)
        if (this.ownerId !== networkManager.currentPlayer?.id) {
            console.warn('⚠️ Skipping AI for remote minion');
            return; // Don't run AI for remote minions at all
        }

        // Delay AI start by 200ms to ensure roles are assigned first
        this.scene.time.delayedCall(200, () => {
            // PERFORMANCE: Increased AI frequency from 100ms to 250ms (4 updates/sec instead of 10)
    ...
    updateAI() {
        ...
        // PERFORMANCE: Removed performance diagnostic (was wasting cycles)
        ...
        // PERFORMANCE: Removed owner verification console.warn
        ...
        // PERFORMANCE: Cache enemy search results for 500ms instead of searching every update
        const now = Date.now();
        if (!this.cachedEnemySearchTime || now - this.cachedEnemySearchTime > 500) {
            this.cachedNearestEnemy = this.findNearestEnemyToSelf(250);
            this.cachedEnemySearchTime = now;
        }

        // Priority 4: Look for enemies near THIS minion (only if close to player)
        const enemy = this.cachedNearestEnemy;
    ...
    findNearestEnemy(searchRadius) {
        ...
        if (this.aggroedEnemies.size >= this.maxAggro) {
            ...
            const alreadyTargeted = Array.from(this.aggroedEnemies)
                .map(id => this.scene.enemies[id] || this.scene.swordDemons[id] || this.scene.minotaurs[id] || this.scene.mushrooms[id] || this.scene.emberclaws[id])
    ...
    die() {
        ...
        // Clear all bleed timers to prevent memory leak
        if (this.bleedTimers && this.bleedTimers.length > 0) {
            this.bleedTimers.forEach(timer => clearInterval(timer));
            this.bleedTimers = [];
        }

        // Clear attack animation timeout to prevent memory leak
        if (this.attackAnimTimeout) {
            clearTimeout(this.attackAnimTimeout);
    ...
    sendPositionUpdate() {
        ...
        // PERFORMANCE: Delta compression - only send if position changed by at least 1 tile
        if (this.lastSentX === currentX && this.lastSentY === currentY) {
            return; // Skip update - hasn't moved significantly
        }

        this.lastSentX = currentX;
        this.lastSentY = currentY;

        const gridPosition = { x: currentX, y: currentY };
