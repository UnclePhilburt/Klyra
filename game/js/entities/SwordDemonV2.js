...
class SwordDemon {
    ...
    setTargetPosition(x, y) {
        ...
        // DEBUG: Log when target is set for tracked enemy
        if (this.data.id === window.debugEnemyId) {
            console.log(`🎯 NEW TARGET: (${x.toFixed(1)}, ${y.toFixed(1)}) [was: (${this.targetX?.toFixed(1)}, ${this.targetY?.toFixed(1)})]`);
        }
    ...
    update() {
        ...
        // DEBUG: Track one specific enemy for detailed logging
        if (!window.debugEnemyId) {
            window.debugEnemyId = this.data.id;
            console.log(`🎯 Tracking SwordDemon: ${this.data.id.slice(-8)}`);
        }
        if (this.data.id === window.debugEnemyId) {
            if (!this.lastLogTime || Date.now() - this.lastLogTime > 100) {
                const dist = Math.sqrt(Math.pow(this.targetX - this.sprite.x, 2) + Math.pow(this.targetY - this.sprite.y, 2));
                console.log(`⚔️ target=(${this.targetX.toFixed(1)}, ${this.targetY.toFixed(1)}) pos=(${this.sprite.x.toFixed(1)}, ${this.sprite.y.toFixed(1)}) dist=${dist.toFixed(1)}`);
        ...
        // Smooth movement towards target using direct lerp (no physics velocity)
        if (this.targetX !== undefined && this.targetY !== undefined) {
            ...
            if (dist > 1) {
                ...
                // DEBUG: Log lerp execution
                if (this.data.id === window.debugEnemyId && (!this.lastLerpLog || Date.now() - this.lastLerpLog > 500)) {
                    console.log(`🔄 LERPING: (${this.sprite.x.toFixed(1)}, ${this.sprite.y.toFixed(1)}) -> (${newX.toFixed(1)}, ${newY.toFixed(1)}) [dist=${dist.toFixed(1)}]`);
                    this.lastLerpLog = Date.now();
                }

                // Set position directly (will auto-sync body since body.moves = false)
                this.sprite.x = newX;
                this.sprite.y = newY;

                // Force body to match sprite position
                if (this.sprite.body) {
                    this.sprite.body.position.x = newX - this.sprite.body.halfWidth;
                    this.sprite.body.position.y = newY - this.sprite.body.halfHeight;
                    this.sprite.body.updateCenter();
                }
            } else if (this.data.id === window.debugEnemyId && dist > 0.01) {
        ...
        // DEBUG: Log final position at end of update
        if (this.data.id === window.debugEnemyId && (!this.lastEndLog || Date.now() - this.lastEndLog > 500)) {
            console.log(`✅ END UPDATE: pos=(${this.sprite.x.toFixed(1)}, ${this.sprite.y.toFixed(1)})`);
            this.lastEndLog = Date.now();
        }
    }
