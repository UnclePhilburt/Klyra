...
class Minotaur {
    ...
    setTargetPosition(x, y) {
        ...
        if (Math.random() < 0.05) {
            console.log(`🎯 Minotaur setTargetPosition: (${x}, ${y})`);
        }
    ...
    update() {
        ...
        if (Math.random() < 0.01) {
            console.log(`🐂 Minotaur update: targetX=${this.targetX}, targetY=${this.targetY}, pos=(${Math.round(this.sprite.x)}, ${Math.round(this.sprite.y)})`);
        }
        ...
        if (this.targetX !== undefined && this.targetY !== undefined) {
            ...
            if (dist > 1) {
                ...
                // Set position directly and sync body
                this.sprite.x = newX;
                this.sprite.y = newY;

                // Sync physics body position manually
                if (this.sprite.body) {
                    this.sprite.body.x = newX - this.sprite.body.halfWidth;
                    this.sprite.body.y = newY - this.sprite.body.halfHeight;
                }
            }
