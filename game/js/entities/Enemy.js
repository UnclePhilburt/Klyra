...
class Enemy {
    constructor(scene, data) {
        ...
        this.pullForce = null; // External pull force (e.g., from Lunare's vortex)
    ...
    showBloodEffect() {
        // PERFORMANCE: Reduced splash count from 3-5 to 1-2
        const splashCount = 1 + Math.floor(Math.random() * 2); // 1-2 blood splashes
        ...
        // PERFORMANCE: Reduced puddles - only spawn 50% of the time
        if (Math.random() < 0.5) {
            ...
            // Initialize permanent blood puddles array if needed
            if (!this.scene.permanentBloodPuddles) {
                this.scene.permanentBloodPuddles = [];
            }

            // Add to permanent collection
            this.scene.permanentBloodPuddles.push(puddle);

            // PERFORMANCE: Limit total puddles to 200 max (prevent memory leak)
            if (this.scene.permanentBloodPuddles.length > 200) {
                const removed = this.scene.permanentBloodPuddles.shift();
                if (removed && removed.destroy) {
                    removed.destroy();
                }
            }

            // PERFORMANCE: Fade out completely and destroy after 30 seconds
            this.scene.tweens.add({
                targets: puddle,
                alpha: 0,
                duration: 30000, // 30 seconds instead of 60
                ease: 'Linear',
                onComplete: () => {
                    puddle.destroy();
                    // Remove from array
                    const index = this.scene.permanentBloodPuddles.indexOf(puddle);
                    if (index > -1) {
                        this.scene.permanentBloodPuddles.splice(index, 1);
                    }
                }
            });
        }
    ...
    updateInterpolation() {
        ...
        // Apply pull force if present (overrides normal movement)
        if (this.pullForce) {
            const oldX = this.sprite.x;
            const oldY = this.sprite.y;
            this.sprite.x += this.pullForce.x;
            this.sprite.y += this.pullForce.y;

            // Debug first pull
            if (Math.random() < 0.01) {
                console.log(`🌀 Enemy pulled from (${oldX.toFixed(0)}, ${oldY.toFixed(0)}) to (${this.sprite.x.toFixed(0)}, ${this.sprite.y.toFixed(0)}), force: (${this.pullForce.x.toFixed(2)}, ${this.pullForce.y.toFixed(2)})`);
            }
            return; // Don't do normal interpolation while being pulled
        }
