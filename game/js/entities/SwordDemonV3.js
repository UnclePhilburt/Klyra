...
class SwordDemon {
    ...
    update() {
        ...
        // ULTRA DEBUG: Always log for first enemy
        if (!window.trackedEnemy) {
            window.trackedEnemy = this.data.id;
            console.log(`🔥 V3 TRACKING: ${this.data.id.slice(-12)}`);
        }

        if (this.data.id === window.trackedEnemy && (!this.logTimer || Date.now() - this.logTimer > 200)) {
            console.log(`🔥 V3 UPDATE CALLED: target=(${Math.round(this.targetX)},${Math.round(this.targetY)}) pos=(${Math.round(this.sprite.x)},${Math.round(this.sprite.y)})`);
