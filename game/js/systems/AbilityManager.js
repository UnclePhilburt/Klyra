...
class AbilityManager {
    ...
    initializeCharacterHandler() {
        ...
        switch (characterId) {
            ...
            case 'ORION':
                if (typeof OrionAbilityHandler !== 'undefined') {
                    this.characterHandler = new OrionAbilityHandler(this.scene, this.player, this);
                } else {
                    console.error('❌ OrionAbilityHandler class not found');
                }
                break;

            case 'LUNARE':
                if (typeof LunareAbilityHandler !== 'undefined') {
                    this.characterHandler = new LunareAbilityHandler(this.scene, this.player, this);
                } else {
                    console.error('❌ LunareAbilityHandler class not found');
                }
                break;

            case 'BASTION':
                if (typeof BastionAbilityHandler !== 'undefined') {
                    this.characterHandler = new BastionAbilityHandler(this.player, this.scene);
                } else {
                    console.error('❌ BastionAbilityHandler class not found');
                }
                break;
    ...
    createCooldownUI() {
        ...
        // Position at bottom center (moved up to avoid overlap)
    ...
    update(time, delta) {
        ...
        // Update character handler (e.g., Bastion's auto-attack system)
        if (this.characterHandler && typeof this.characterHandler.update === 'function') {
            // Gather all enemies from the scene (convert objects to arrays)
            const allEnemies = [
            ...
            this.characterHandler.update(time, allEnemies);
        }
    ...
    updateCooldownUI() {
        if (!this.cooldownUI) {
            console.log('⚠️ updateCooldownUI: cooldownUI is null');
            return;
        }
    ...
    repositionUI() {
        ...
        const centerX = width / 2;
        const bottomY = height - (140 * uiScale);
